# from pathlib import Path
# import pytest
# import yaml
# import allure
#
# # 直接从子模块导入，避免 package __init__ 导出问题
# from pageobjects.account.LoginPage import LoginPage
#
# PROJECT_ROOT = Path(__file__).resolve().parents[2]
# DATA_DIR = PROJECT_ROOT / "data" / "account"
#
# # def _load_cases(yaml_file: Path):
# #     cases = yaml.safe_load(yaml_file.read_text(encoding="utf-8")) or []
# #     params = []
# #     for c in cases:
# #         # 将 yaml 里 marks 转成 pytest marks（若不存在会报 warning，但不影响运行）
# #         marks = [getattr(pytest.mark, m) for m in c.get("marks", [])]
# #         params.append(pytest.param(c, id=c["id"], marks=marks))
# #     return params
#
# def _load_cases(yaml_file: Path):
#     cases = yaml.safe_load(yaml_file.read_text(encoding="utf-8")) or []
#     params = []
#     for c in cases:
#         marks = []
#         for m in c.get("marks", []):
#             mark = getattr(pytest.mark, m, None)
#             if mark is not None:
#                 marks.append(mark)
#             else:
#                 # 可选：记录一个警告/日志，帮助定位 YAML 中拼写错误的 mark
#                 # import warnings; warnings.warn(f"Unknown mark '{m}' in {yaml_file}")
#                 pass
#         params.append(pytest.param(c, id=c["id"], marks=marks))
#     return params
#
# @allure.feature("Account")
# @allure.story("Login")
# @pytest.mark.smoke
# @pytest.mark.parametrize("case", _load_cases(DATA_DIR / "login.yaml"))
# def test_login_smoke(case, driver, base_url, config):
#     """
#     P0 冒烟：登录正/反向用例
#     使用 fixtures:
#       - driver: selenium webdriver
#       - base_url: 从 environment.yaml 读取的 base_url
#       - config: 环境配置（可用于取默认用户名/密码）
#     """
#     allure.dynamic.title(f'{case["id"]} {case["title"]}')
#
#     page = LoginPage(driver, base_url).open()
#
#     # 优先使用用例数据，如果用例中未给 username/password，可回退到 environment.yaml 中的配置
#     username = case["input"].get("username") or config.get("username")
#     password = case["input"].get("password") or config.get("password")
#     # remember = case["input"].get("remember", False)
#     remember = case["input"].get("remember", case["input"].get("remember_me", False))
#
#     with allure.step("填写并提交登录"):
#         page.login(username=username, password=password, remember=remember)
#
#     with allure.step("断言登录结果"):
#         if case["expected"].get("ok"):
#             page.assert_login_success()
#         else:
#             expected_msg = case["expected"].get("message_contains", "")
#             page.assert_error_contains(expected_msg)

#
# from pathlib import Path
# import pytest
# import yaml
# import allure
#
# # 直接从子模块导入，避免 package __init__ 导出问题
# from pageobjects.account.LoginPage import LoginPage
#
# PROJECT_ROOT = Path(__file__).resolve().parents[2]
# DATA_DIR = PROJECT_ROOT / "data" / "account"
#
# def _load_cases(yaml_file: Path):
#     cases = yaml.safe_load(yaml_file.read_text(encoding="utf-8")) or []
#     params = []
#     for c in cases:
#         marks = []
#         for m in c.get("marks", []):
#             mark = getattr(pytest.mark, m, None)
#             if mark is not None:
#                 marks.append(mark)
#         params.append(pytest.param(c, id=c["id"], marks=marks))
#     return params
#
# @allure.feature("Account")
# @allure.story("Login")
# @pytest.mark.parametrize("case", _load_cases(DATA_DIR / "login.yaml"))
# def test_login(case, driver, base_url, config):
#     """
#     使用 param-level marks（来自 login.yaml 的 marks 字段）来区分 smoke / negative / regression。
#     不在函数层级添加 pytest.mark.*，运行时使用 -m 过滤需要的用例集。
#      P0 冒烟：登录正/反向用例
#          使用 fixtures:
#            - driver: selenium webdriver
#            - base_url: 从 environment.yaml 读取的 base_url
#            - config: 环境配置（可用于取默认用户名/密码）
#     """
#
#     allure.dynamic.title(f'{case["id"]} {case["title"]}')
#
#     page = LoginPage(driver, base_url).open()
#
#     # 兼容 remember 或 remember_me 两种写法
#     username = case["input"].get("username") or config.get("username")
#     password = case["input"].get("password") or config.get("password")
#     remember = case["input"].get("remember", case["input"].get("remember_me", False))
#
#     with allure.step("填写并提交登录"):
#         page.login(username=username, password=password, remember=remember)
#
#     with allure.step("断言登录结果"):
#         if case["expected"].get("ok"):
#             page.assert_login_success()
#         else:
#             expected_msg = case["expected"].get("message_contains", "")
#             page.assert_error_contains(expected_msg)

#
# from pathlib import Path
# import pytest
# import yaml
# import allure
#
# # 直接从子模块导入，避免 package __init__ 导出问题
# from pageobjects.account.LoginPage import LoginPage
#
# PROJECT_ROOT = Path(__file__).resolve().parents[2]
# DATA_DIR = PROJECT_ROOT / "data" / "account"
#
# def _load_cases(yaml_file: Path):
#     cases = yaml.safe_load(yaml_file.read_text(encoding="utf-8")) or []
#     params = []
#     for c in cases:
#         marks = []
#         for m in c.get("marks", []):
#             mark = getattr(pytest.mark, m, None)
#             if mark is not None:
#                 marks.append(mark)
#         params.append(pytest.param(c, id=c["id"], marks=marks))
#     return params
#
# @allure.feature("Account")
# @allure.story("Login")
# @pytest.mark.parametrize("case", _load_cases(DATA_DIR / "login.yaml"))
# def test_login(case, driver, base_url, config):
#     """
#        P0 冒烟：登录正/反向用例
#        使用 fixtures:
#          - driver: selenium webdriver
#          - base_url: 从 environment.yaml 读取的 base_url
#          - config: 环境配置（可用于取默认用户名/密码）
#     """
#     allure.dynamic.title(f'{case["id"]} {case["title"]}')
#
#     page = LoginPage(driver, base_url).open()
#
#     # 获取对应的用户名/密码
#     username = case["input"].get("username") or config.get("username")
#     password = case["input"].get("password") or config.get("password")
#     remember = case["input"].get("remember", False)
#     # remember = case["input"].get("remember", case["input"].get("remember_me", False))
#
#     with allure.step("填写并提交登录"):
#         page.login(username=username, password=password, remember=remember)
#
#     with allure.step("断言登录结果"):
#         if case["expected"].get("ok"):
#             page.assert_login_success()
#         else:
#             expected_msg = case["expected"].get("message_contains", "")
#             page.assert_error_contains(expected_msg)


# from pathlib import Path
# import pytest
# import yaml
# import allure
#
# # 直接从子模块导入，避免 package __init__ 导出问题
# from pageobjects.account.LoginPage import LoginPage
#
# PROJECT_ROOT = Path(__file__).resolve().parents[2]
# DATA_DIR = PROJECT_ROOT / "data" / "account"
#
# def _load_cases(yaml_file: Path):
#     """
#     从 yaml 加载用例列表，并把用例里定义的 marks 转为 pytest.param 的 marks。
#     使用 getattr(..., None) 来避免不存在的 mark 导致异常。
#     """
#
#     cases = yaml.safe_load(yaml_file.read_text(encoding="utf-8")) or []
#     params = []
#     for c in cases:
#         marks = []
#         for m in c.get("marks", []):
#             mark = getattr(pytest.mark, m, None)
#             if mark is not None:
#                 marks.append(mark)
#         params.append(pytest.param(c, id=c["id"], marks=marks))
#     return params
#
# @allure.feature("Account")
# @allure.story("Login")
# @pytest.mark.parametrize("case", _load_cases(DATA_DIR / "login.yaml"))
# def test_login(case, driver, base_url, config):
#     """
#            P0 冒烟：登录正/反向用例
#            使用 fixtures:
#              - driver: selenium webdriver
#              - base_url: 从 environment.yaml 读取的 base_url
#              - config: 环境配置（可用于取默认用户名/密码）
#         """
#     allure.dynamic.title(f'{case["id"]} {case["title"]}')
#
#     page = LoginPage(driver, base_url).open()
#
#     # 兼容 remember 或 remember_me 两种写法
#     username = case["input"].get("username") or config.get("username")
#     password = case["input"].get("password") or config.get("password")
#     remember = case["input"].get("remember", case["input"].get("remember_me", False))
#
#     with allure.step("填写并提交登录"):
#         page.login(username=username, password=password, remember=remember)
#
#     with allure.step("断言登录结果"):
#         if case["expected"].get("ok"):
#             # 若用例期望成功，可传入期望的 landing 路径供断言使用（如果 LoginPage 支持）
#             landing = case["expected"].get("landing")
#             # LoginPage.assert_login_success 应按项目实现进行断言；这里尽量把 landing 传入以便更精确校验
#             try:
#                 if landing:
#                     page.assert_login_success(landing)
#                 else:
#                     page.assert_login_success()
#             except TypeError:
#                 # 兼容旧版 assert_login_success 不接收参数的实现
#                 page.assert_login_success()
#         else:
#             expected_msg = case["expected"].get("message_contains", "")
#             selector = case["expected"].get("selector")
#             timeout = case["expected"].get("wait", 3)
#             page.assert_error_contains(expected_msg, selector=selector, timeout=timeout)




from pathlib import Path
import pytest
import yaml
import allure

from pageobjects.account.LoginPage import LoginPage

PROJECT_ROOT = Path(__file__).resolve().parents[2]
DATA_DIR = PROJECT_ROOT / "data" / "account"


def _load_cases(yaml_file: Path):
    cases = yaml.safe_load(yaml_file.read_text(encoding="utf-8")) or []
    params = []
    for c in cases:
        marks = []
        for m in c.get("marks", []):
            mark = getattr(pytest.mark, m, None)
            if mark is not None:
                marks.append(mark)
        params.append(pytest.param(c, id=c["id"], marks=marks))
    return params

#
# @allure.feature("Account")
# @allure.story("Login")
# @pytest.mark.parametrize("case", _load_cases(DATA_DIR / "login.yaml"))
# def test_login(case, driver, base_url, config):
#     """
#     冒烟 + 回归：登录正/反向用例
#     使用 fixtures:
#       - driver: selenium webdriver
#       - base_url: 从 environment.yaml 读取
#       - config: 默认账号配置
#     """
#     allure.dynamic.title(f'{case["id"]} {case["title"]}')
#
#     page = LoginPage(driver, base_url).open()
#
#     # 输入参数兼容
#     username = case["input"].get("username") or config.get("username")
#     password = case["input"].get("password") or config.get("password")
#     remember = case["input"].get("remember", case["input"].get("remember_me", False))
#
#     with allure.step("填写并提交登录"):
#         page.login(username=username, password=password, remember=remember)
#
#     with allure.step("断言登录结果"):
#         if case["expected"].get("ok"):
#             landing = case["expected"].get("landing")
#             try:
#                 if landing:
#                     page.assert_login_success(landing)
#                 else:
#                     page.assert_login_success()
#             except TypeError:
#                 page.assert_login_success()
#         else:
#             expected_msg = case["expected"].get("message_contains", "")
#             selector = case["expected"].get("selector")
#             timeout = case["expected"].get("wait", 3)
#
#             # 先尝试 JS alert，再回退页面元素
#             alert_handled = False
#             try:
#                 page.assert_alert_contains(expected_msg, timeout=timeout)
#                 alert_handled = True
#             except Exception:
#                 # 无 alert 或 alert 文案不匹配则回退
#                 pass
#
#             if not alert_handled:
#                 page.assert_error_contains(expected_msg, selector=selector, timeout=timeout)



@allure.feature("Account")
@allure.story("Login")
@pytest.mark.parametrize("case", _load_cases(DATA_DIR / "login.yaml"))
def test_login(case, driver, base_url, config):
    """
    冒烟 + 回归：登录正/反向用例
    使用 fixtures:
      - driver: selenium webdriver
      - base_url: 从 environment.yaml 读取
      - config: 默认账号配置
    """
    allure.dynamic.title(f'{case["id"]} {case["title"]}')

    page = LoginPage(driver, base_url).open()

    # 仅当键不存在时才回退到 config；若键存在（哪怕是空字符串）就使用用例中的值
    username = case["input"]["username"] if "username" in case["input"] else config.get("username")
    password = case["input"]["password"] if "password" in case["input"] else config.get("password")
    remember = case["input"].get("remember", case["input"].get("remember_me", False))

    with allure.step("填写并提交登录"):
        page.login(username=username, password=password, remember=remember)

    with allure.step("断言登录结果"):
        if case["expected"].get("ok"):
            landing = case["expected"].get("landing")
            try:
                if landing:
                    page.assert_login_success(landing)
                else:
                    page.assert_login_success()
            except TypeError:
                page.assert_login_success()
        else:
            expected_msg = case["expected"].get("message_contains", "")
            selector = case["expected"].get("selector")
            timeout = case["expected"].get("wait", 3)

            # 先尝试 JS alert，再回退页面元素
            alert_handled = False
            try:
                page.assert_alert_contains(expected_msg, timeout=timeout)
                alert_handled = True
            except Exception:
                pass

            if not alert_handled:
                page.assert_error_contains(expected_msg, selector=selector, timeout=timeout)